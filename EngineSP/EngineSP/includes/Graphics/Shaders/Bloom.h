...
namespace SP
{
    ...
    {
        SP_INLINE BloomShader(const std::string& path, int32_t width, int32_t height) :
            Shader(path), m_Width(width), m_Height(height)
        {
            ...
            for (auto i = 0; i < 2; i++)
            {
                ...
                {
                    spdlog::error("BloomShader() Failed!");
                }
            }
        ...
        SP_INLINE void Compute(uint32_t brightnessMap, uint32_t stepCount)
        {
            // bind shader program
