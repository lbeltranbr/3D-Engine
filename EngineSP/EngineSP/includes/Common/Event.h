...
namespace SP
{
    ...
    struct EventListener
    {
        ...
        SP_INLINE EventListener(CallbackFn&& callback, uint32_t listnrid) :
    ...
    struct EventDispatcher
    {
        ...
        SP_INLINE void AttachCallback(Callback&& callback, uint32_t listnrid)
        {
        ...
        template <typename Event>
        SP_INLINE void DetachCallback(uint32_t listnrid)
        {
        ...
        SP_INLINE void EraseListener(uint32_t listnrid)
        {
    ...
    private:
        template <typename Event>
        SP_INLINE EventRegistry<Event>* CastRegistry(void* p)
        {
            return static_cast<EventRegistry<Event>*>(p);
        }

        template <typename Event>
        SP_INLINE EventRegistry<Event>* GetRegistry()
        {
